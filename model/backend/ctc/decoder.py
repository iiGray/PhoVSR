...
#         self.logzero=-torch.inf
    ...
    def selectNext(self, bk_pref, bk_pred):
        ...
        # self.prefP=self.prefP[bk_pref,bk_pred].unsqueeze(1).repeat(1,self.vocab_size)
        # self.lastP=self.lastP[:, :, bk_pref, self.vocabMap[bk_pref,bk_pred]]
    ...
    def scoring(self,cState: State, dState: State, ctc_beam_idx):
        ...
        # (batch_beam_size, ctc_beam_size) -  (batch_beam_size, 1)
        # curP=P.logsumexp(dim=[0])[start:end].logsumexp(dim=[0])- \
        #     (0. if end==1 else \
        #         self.prefP[max(start-1,0):end-1].logsumexp(dim=0))
        ...
        # phi= torch.cat((P[:,[0]],P[:,:-1]),dim=1)[0]+X[0]
        # curP = torch.logsumexp(
        #         torch.cat((phi[start:end], P[0, start - 1].unsqueeze(0)), dim=0),
        #         dim=0,
        #     )
        
        # add <eos> prob
#         curP[:,self.eos]=lastPsum[num_steps-1,:]
#         curP[:,self.blank]=self.logzero
...
class PhoneticCTCdecoder(nn.Module,EvalModule):
    def __init__(self,
                 idims,
                 char_vocab,
                 phoneme_vocab,
        ...
        # mat: for each pinyin, it has different char
        # c2p: char idx to pinyin idx
    ...
    def initState(self, eState: State):
        ...
        self.ctc_arange=torch.arange(self.map.size(0),device=eState.device)
    ...
    def scoring(self,cState: State, dState: State, ctc_bea_idx):
        '''
        ...
        vocabMap    :  vocab_ids(pinyin) to beam_ids
        return: ctc  score         (batch_beam_size, vocab_size)
        '''
        # already selected (batch_beam_size, cur_num_steps) 
        y,yl=dState
        ...
        #(batch_beam_size, 1)
        lastpred=self.c2p[y[:,[-1]]]

        sameidx=(lastpred==ctc_arange_idx).nonzero(as_tuple=True)

        # (num_steps, batch_beam_size, pinyin_ctc_arange_size)
        ...
        finalP=(curP[:,:,None].repeat(1,1,self.map.size(-1)) * self.map[None,:,:]).sum(dim=1)
        finalP[finalP==0]=self.logzero
